...
import { DELETE_USER, GET_ALL_USERS, IDeleteUser, IGetUsers } from "../types/manageUserType";

import { checkImage, imageUpload } from "../../utils/ImageUpload";
import { patchAPI, getAPI, deleteAPI } from "../../utils/FetchData";
...
export const getAllUser =
  () => async (dispatch: Dispatch<IAlertType | IGetUsers>) => {
    try {
      dispatch({ type: ALERT, payload: { loading: true } });

      const res = await getAPI("manage_users");
...
export const deleteUser =
  ...
  async (dispatch: Dispatch<IAlertType | IDeleteUser>) => {
    const result = await checkTokenExp(token, dispatch);
    const access_token = result ? result : token;
    try {
      dispatch({ type: DELETE_USER, payload: id });
      await deleteAPI(`user/${id}`, access_token);
    } catch (err: any) {
      dispatch({ type: ALERT, payload: { errors: err.response.data.msg } });
    }
  };
